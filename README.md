...
LinkSpire is a robust, decoupled RESTful API backend designed for URL shortening and request analytics. Built using **Node.js, Express, and MySQL**, this service manages redirection routing, collision-safe code generation, database connection pooling, and live request analytics.
...
- **Dynamic Configuration (Env Variables)**: Production-ready setups supporting the project's lowercase environment variables (e.g. `mysql_host`, `mysql_user`).
...
2. Install Node modules:
   ...
   npm install
   ```
3. Setup environmental variables in a local `.env` file using the project's lowercase convention:
   ```env
...
2. Configure your cloud database connection credentials via lowercase environment variables on your hosting provider dashboard:
   - `mysql_host`: Your remote database host address
   - `mysql_user`: Your remote database username (e.g. `avnadmin` on Aiven)
   - `mysql_password`: Your remote database password
   - `mysql_database`: Your database name (e.g. `defaultdb` on Aiven)
   - `mysql_port`: Your remote database port (e.g. `12345`)
3. The database pool automatically initiates SSL handshakes (`ssl: { rejectUnauthorized: false }`) to support secure hosting services like Aiven.io.
